...
<script type="text/javascript">
    ...
    $(".type_inp").change(function () {
      var colname = $(this).val();
      
      $.ajax({
           type:'GET',
           url:"{{ route('massdispcombo') }}",
           data:{"sendData": colname},
           success:function(data){
              //Success code
              let isCombo = data[0];
              // let isCombo = JSON.parse(text);
              if (isCombo.type == 2 && flag == false) {
                flag = true;
                $(".section-body").append(`<div class="container sub-container">
    ...
    $BTN.click(function () {
      ...
      $.ajax({
           type:'GET',
           url:"{{ route('massupdatecol') }}",
           data:{"sendData": sendData, "sendData2": sendData2},
           success:function(data){
              //Success code
              if (data) {
                alert("Success Update Columns!");
